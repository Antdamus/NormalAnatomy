...
- `media/`: all 44 clinical image files, copied unchanged from Downloads and named exactly as the corrected image references.
...
Text import does not embed image bytes. The files in `media/` must be present by these exact basenames in the active Anki profile's `collection.media` directory when studying. No populated Anki profile was found at the conventional local path, so collection media installation, live note-type inspection, and live Anki import were not claimed. [Anki media import documentation](https://docs.ankiweb.net/importing/text-files.html#importing-media).
...
- Six distinct `arrow_*.png` caption icons were unlisted/unavailable. Their broken inline image tags were replaced with readable `(arrow)` cues. All actual clinical images and their embedded annotations were preserved.
...
assert refs=={x['filename'] for x in media}
for entry in media:assert hashlib.sha256((B/'media'/entry['filename']).read_bytes()).hexdigest().upper()==entry['sha256']
assert sum(e['matches_metadata_plain_evidence'] is True for e in media)==22
hashes={name:hashlib.sha256((B/name).read_bytes()).hexdigest() for name in ['corrected_cards.tsv','corrected_cards_anki_import.tsv','corrected_cards_anki_create_deck.tsv','audit_report.md','media_integrity.json']}
