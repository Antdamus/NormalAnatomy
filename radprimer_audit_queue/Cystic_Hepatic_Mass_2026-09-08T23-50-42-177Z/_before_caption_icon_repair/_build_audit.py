...
arrow_re=re.compile(r'<img\b[^>]*src=[\"\']arrow_[^\"\']+[\"\'][^>]*>',re.I)
def clean(text):
    for old,new in rename.items(): text=text.replace(old,new)
    return arrow_re.sub('(arrow)',text)
...
assert refs==expected and all((B/'media'/fn).is_file() for fn in refs)
...
assert not re.search(r'Image references?:|Source image links?:|https?://|arrow_\w+\.png','\n'.join('\t'.join(r[:21]) for r in out),re.I)
