[CmdletBinding()]
param(
  [string]$SourceDir = "$env:USERPROFILE\Downloads\RadiologyMasterSource",
  [string]$DestinationDir = "$env:USERPROFILE\NormalAnatomy\master_source_queue"
)
...
$latest = Get-ChildItem -LiteralPath $SourceDir -Directory |
  Where-Object { Test-Path -LiteralPath (Join-Path $_.FullName "_master_source_request_complete.txt") } |
...
Set-Content -LiteralPath (Join-Path $DestinationDir "_latest_master_source_bundle.txt") -Value $destinationBundle
