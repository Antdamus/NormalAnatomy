...
function buildImaiosLabelRepositoryChunkPrompt(repository) {
  ...
  const promptText = [
    ...
    "- never use generic chunk titles such as `Chunk 1`, `Chunk 5`, `Group 2`, or `Section`; every JSON chunk title must name the anatomy, for example `Ossicular chain: malleus`, `Middle ear and mastoid: air-space pathway`, or `Craniovertebral junction: dens stabilizers`",
    ...
    "- include a high-quality concise learningFrame on each JSON chunk; this is the learner's scan guide while reviewing the live image",
    "- learningFrame must preserve image-recognition quality: include 3-5 short action-oriented lines covering orientation/scan order, key boundaries or relationships, and a nearby confusion/checkpoint when supported",
    "- in each chunk, spell out less-universal anatomy, pathology, ligament, procedure, or mechanism acronyms the first time they appear, followed by the acronym in parentheses; after that, the acronym may be used in the same chunk",
    "- do not force expansion of universally familiar imaging modality acronyms such as CT or MRI; these may stay compact in titles, module metadata, and learningFrame prose",
    "- include 1-3 article/source-informed teaching points only when the source clearly emphasizes why that anatomy matters; source points must supplement, not replace, the scan-review guide",
    "- avoid vague filler such as `review this area`, `center the review`, or `identify the structures` unless followed by a concrete landmark, boundary, relationship, or visual comparison",
...
function buildImaiosFinalChunkPromptFromHarvest(harvestResult) {
  ...
  const promptText = [
    ...
    "- Every JSON chunk must have a descriptive anatomy title. Never use generic titles such as `Chunk 1`, `Chunk 5`, `Group 2`, or `Section`.",
    "- Chunk titles should name the larger region and subfocus, for example `Ossicular chain: malleus`, `Middle ear and mastoid: air-space pathway`, or `Craniovertebral junction: dens stabilizers`.",
    ...
    "- Produce a high-quality concise learningFrame array for every chunk; aim for 3-5 short action-oriented lines.",
    "- Cover orientation/scan order, key landmarks, boundaries or relationships, contents/pathways when relevant, and what to compare on the image.",
    "- Add a nearby confusion/checkpoint when supported by definitions or source context, especially normal variants, pseudolesions, adjacent compartments, or subtle boundaries.",
    "- In each chunk, spell out less-universal anatomy, pathology, ligament, procedure, or mechanism acronyms the first time they appear, followed by the acronym in parentheses; after that, the acronym may be used in the same chunk.",
    "- Do not force expansion of universally familiar imaging modality acronyms such as CT or MRI; these may stay compact in titles, module metadata, and learningFrame prose.",
    "- Include 1-3 article/source-informed teaching points when the source clearly emphasizes a structure, route, mimic, danger landmark, reporting boundary, or mechanism-relevant landmark; these must supplement the scan guide, not replace it.",
...
function buildSourceOnlyWakeMessage(bundle) {
  ...
  return [
    ...
    "",
    "Please import the newest complete RadPrimer audit bundle, read the queue pointer, then compare the provided TSV against the source package. Preserve the TSV schema and column order, remove weak/meta cards, split overloaded cards, preserve short non-diagnostic random-looking Clinical_Context IDs, repair missing/counter-style IDs, reject sequential IDs such as Q00000000001, apply the radiologist-usefulness gate to High-Yield cards, keep differential lists on UNKNOWN backs without creating standalone Differential Drill cards unless Differential_Q is explicitly populated, add only source-supported high-yield cards that improve image interpretation, and produce corrected_cards.tsv plus audit_report.md. If metadata.json contains an Anki deck target, also produce corrected_cards_anki_import.tsv."
  ].join("\n");
}
