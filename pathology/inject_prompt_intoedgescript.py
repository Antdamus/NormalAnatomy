...
def escape_for_js_template_literal(s: str) -> str:
    ...
    s = s.replace("\\", "\\\\")      # preserve backslashes
    s = s.replace("`", "\\`")        # don't break template literal
    s = s.replace("${", "\\${")      # prevent JS interpolation
    return s
...
def main():
    ...
    input_js_path = Path("input_code.js")  # your big JS snippet file
